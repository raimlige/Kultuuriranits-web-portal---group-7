...
export default async function ProgramPage({
  ...
  const [program, currentUser] = await Promise.all([
    getProgram(id),
    getCurrentUser(),
  ]);
        ...
        <div className="space-y-6">
          ...
          {isTeacher && currentUser && (
            <div className="bg-white rounded-2xl p-6 sm:p-8 border border-gray-150 shadow-sm hover:shadow-md transition-shadow duration-300">
              <div className="mb-6">
                <h3 className="text-lg font-extrabold text-gray-900 tracking-tight mb-1 flex items-center gap-2">
                  <MessageSquarePlus className="w-5 h-5 text-blue-600" />
                  Tagasiside
                </h3>

                <p className="text-xs font-medium text-gray-500">
                  Jaga oma kogemust antud haridusprogrammiga.
                </p>
              </div>
